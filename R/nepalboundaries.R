...
#'
#' Retrieves national park and protected area boundaries of Nepal.
#'
#' @param park Character vector. Optional national park / protected area name(s).
...
nb_nationalpark <- function(park = NULL) {
  ...
  if (!"park_name" %in% names(data) && "NAME" %in% names(data)) {
    names(data)[names(data) == "NAME"] <- "park_name"
  }
